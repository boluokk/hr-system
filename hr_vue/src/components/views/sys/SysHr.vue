<template>
  <div>
    ...
    <el-card class="box-card lightColor" :style="fatherCard">
      ...
      <div class="hrbox">
        <el-card class="box-card" shadow="hover" style="position: relative;width: 300px;height: 400px;display:inline-block;margin-left:40px;margin-bottom:50px;" v-for="(item,idx) in initData" :key="idx">
        <div
          slot="header"
          class="clearfix"
...
<el-drawer
  ...
  <div>
    ...
    <div class="demo-drawer__footer" style="width: 200px;margin: 0 50%;transform: translateX(-50%);">
      ...
      <el-button type="primary" size="small" @click="$refs.drawer.closeDrawer()" :loading="loading">{{ loading ? '提交中 ...' : '确 定' }}</el-button>
    </div>
  </div>
</el-drawer>
  </div>
